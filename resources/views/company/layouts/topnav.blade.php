        ...
        <li class="nav-item nav-profile dropdown">
          <a class="nav-link dropdown-toggle p-3" id="profileDropdown" href="#" data-bs-toggle="dropdown" aria-expanded="false">
            ...
            @if(auth()->user()->company->logo)
                <img src="{{asset('storage/'.auth()->user()->company->logo)}}" alt="Profile" class="rounded-circle" style="width: 50px; height: 50px;">
            @endif
            </div>
              <p class="d-none d-md-block mt-3 mx-1">  {{auth()->user()->company->company_name}}</p>
          </a>
